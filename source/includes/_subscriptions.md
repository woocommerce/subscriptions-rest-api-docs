...
| `payment_retry_date_gmt`    | date-time | The subscription's payment retry date in GMT. Note: Only included if the [Failed Recurring Payment Retry System](https://woocommerce.com/document/subscriptions/failed-payment-retry/) is enabled. <i class="label label-info">read-only</i> |
...
| `transition_status`         | string    | The subscription status to transition to. Status transitions will re-calculate the appropriate subscription dates. See [Setting status](#setting-status) <i class="label label-info">write-only</i>                                          |
